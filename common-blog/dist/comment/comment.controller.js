...
let CommentController = class CommentController {
    async index() {
        return await comment_model_1.CommentModel.find();
    }
    async create(createPostDto) {
        await comment_model_1.CommentModel.create(createPostDto);
        return {
            success: true,
        };
    }
    async detail(id) {
        return await comment_model_1.CommentModel.findById(id);
    }
    async update(id, updatePostDto) {
        await comment_model_1.CommentModel.findByIdAndUpdate(id, updatePostDto);
        return {
            success: true,
        };
    }
    async remove(id) {
        await comment_model_1.CommentModel.findByIdAndDelete(id);
        return {
            success: true,
        };
    }
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], CommentController.prototype, "detail", null);
__decorate([
    common_1.Put(':id'),
